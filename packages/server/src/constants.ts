...
// the headless terminal's current state with the official @xterm/addon-serialize
// and sends one `viewport-snapshot` the viewer paints in a single write. Once
// every viewer's buffer drains below the low water the session returns to raw
// passthrough. The high water sits well under the PTY-pause high water so
// render-skip is the first line of defense (send less); the PTY pause only
// triggers if the buffer nonetheless climbs toward the runaway threshold. On
// a fast link (LAN) the buffer never reaches the high water, so render-skip
