...
- 523642f: Raise MAX_PROCESS_REQUESTED_SECRETS from 32 to MAX_SECRETS (64) so a process or automation can reference every configured secret — previously a process with 31 secrets wired (e.g. `pi`) failed to save with a 400 as soon as two more secrets were selected, and the UI collapsed every rejection into one misleading "check the binary name" message.
...
- 51a376b: Resize shared tmux sessions to both natural viewport axes when ownership moves between phone and desktop, and keep on-screen Ctrl-prefix chords contiguous by suppressing programmatic focus reports between keys.
