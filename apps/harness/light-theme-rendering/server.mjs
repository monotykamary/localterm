...
import { DIAGNOSTIC_FONTS } from "./fonts.mjs";
...
const harnessRequire = createRequire(join(harnessDirectory, "package.json"));
...
const canvasAddonDirectory = dirname(harnessRequire.resolve("@xterm/addon-canvas/package.json"));
const fontRoutes = new Map();
const fontStylesheetParts = [];
for (const font of DIAGNOSTIC_FONTS) {
  const fontDirectory = packageDirectory(`@fontsource/${font.id}`);
  for (const weight of new Set([400, font.boldWeight])) {
    const stylesheet = await readFile(join(fontDirectory, `${weight}.css`), "utf8");
    for (const match of stylesheet.matchAll(/\.\/files\/([^),]+)\)/g)) {
      const filename = match[1];
      fontRoutes.set(`/fonts/${font.id}/${filename}`, [
        join(fontDirectory, "files", filename),
        filename.endsWith(".woff2") ? "font/woff2" : "font/woff",
      ]);
    }
    fontStylesheetParts.push(
      stylesheet
        .replaceAll("url(./files/", `url(/fonts/${font.id}/`)
        .replaceAll("font-display: swap", "font-display: block"),
    );
  }
}
const fontStylesheet = fontStylesheetParts.join("\n");
...
const routes = new Map([
  ...
  ["/harness/fonts.mjs", [join(harnessDirectory, "fonts.mjs"), "text/javascript"]],
  ...
  [
    "/addon/canvas.mjs",
    [join(canvasAddonDirectory, "lib/xterm-addon-canvas.mjs"), "text/javascript"],
  ],
  [
    "/themes.mjs",
