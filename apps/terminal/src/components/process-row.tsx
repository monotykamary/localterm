      ...
      <div className="mt-1 flex flex-wrap gap-1">
        {process.requestedSecrets.length === 0 ? (
          ...
          process.requestedSecrets.map((secretName) => (
            <span
              key={secretName}
              className="inline-flex max-w-full min-w-0 items-center gap-1 rounded-sm border border-border/40 bg-muted/30 px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground/90"
            >
              <span className="min-w-0 truncate">{secretName}</span>
