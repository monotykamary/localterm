...
// the session's cwd as a uniquely-named file and returns the absolute path.
// The caller pastes that path (shell-quoted) into the prompt. Throws on a
// non-201 response so the caller can surface the failure — a slow mobile link
// can take a few seconds, and a silent no-op leaves the user wondering why the
// tap did nothing.
